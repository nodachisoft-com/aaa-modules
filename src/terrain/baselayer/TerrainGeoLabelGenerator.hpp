...
namespace a3c
{
  ...
  private:
    ...
    LabelsGeoEntity getRndFromVecLabel(std::vector<LabelsGeoEntity> list, MtRandomizer &rnd)
    {
      ...
      {
        Logger logger;
        logger.errorLog("TerrainGeoLabelGenerator::getRndFromVec(): list.size() is 0.");
      }
      int index = rnd.getRndInt(0, size - 1);
    ...
    LabelsGeoEntity fetchRndFromVecLabel(std::vector<LabelsGeoEntity> &list, MtRandomizer &rnd)
    {
      ...
      {
        Logger logger;
        logger.errorLog("TerrainGeoLabelGenerator::fetchRndFromVecLabel(): list.size() is 0.");
      }
      int index = rnd.getRndInt(0, size - 1);
  ...
  public:
    TerrainGeoLabelGenerator() : dbRef(nullptr), ScaleSmallLabelList({})
    {
    }
